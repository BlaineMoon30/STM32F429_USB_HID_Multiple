...
{
  static uint32_t mem[sizeof(USBD_HID_HandleTypeDef) + sizeof(USBD_HID_HandleTypeDef) + sizeof(USBD_HID_HandleTypeDef) + sizeof(uint32_t) + sizeof(uint32_t)];

  void *alloc = NULL;

  if ((int)p == (int)&USBD_HID)
  {
    alloc = (void*)&mem[0];
  }
  else if ((int)p == (int)&USBD_HID_2)
  {
    alloc = (void*)&mem[(sizeof(USBD_HID_HandleTypeDef) + sizeof(uint32_t)) / sizeof(uint32_t)];
  }
  else if ((int)p == (int)&USBD_HID_3)
    {
      alloc = (void*)&mem[(sizeof(USBD_HID_HandleTypeDef) + sizeof(USBD_HID_HandleTypeDef) + sizeof(uint32_t)) / sizeof(uint32_t)];
    }
